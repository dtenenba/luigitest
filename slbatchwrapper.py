...
logger.level = logging.DEBUG # this can be overridden
...
try:
    ...
    DEFAULT_QUEUE_NAME = 'medium' #list(queues.keys())[0]
...
def random_id():
    return os.getenv('USER') + '-luigi-job-' + ''.join(random.sample(string.ascii_lowercase, 8))
...
class DockerTask(sciluigi.Task):
...
class BatchTask(DockerTask):
    ...
    job_definition = sciluigi.Parameter()
    job_name = sciluigi.Parameter(default='', significant=False)
    queue_name = sciluigi.Parameter(default='', significant=False)
    local = False
    environment = None
    vcpus = None
    memory = None
    ...
    def run(self):
        ...
        if not self._job_id:
            container_overrides = {}
            if self.command:
                container_overrides['command'] = self.command
            if self.environment:
                container_overrides['environment'] = self.environment
            if self.retry_strategy:
                container_overrides['retryStrategy'] = self.retry_strategy
